<template>
    <div class="mg-6 profile-wrapper">
        ...
        <div class="profile-wrap">
            <div class="profile-maintitle"><span class="topic-title">{{ checkout ? 'Reserva de ' + checkout.checkin.reserva.huesped.nombres + ' ' + checkout.checkin.reserva.huesped.apellidos : 'Cargando' }}</span>
            </div>
            <div class="profile-subtitle">ID • {{ checkout ? checkout._id : 'Cargando' }}</div>
                ...
                <div class="profile-left">
                    ...
                    <div class="profile-details">
                        <div class="profile-detail">
                            ...
                            <div class="profile-detail-large">{{ checkout  ? checkout.checkin.reserva.huesped.nombres + " " + checkout.checkin.reserva.huesped.apellidos : 'Cargando' }}
                            </div>
                        </div>
                        <div class="profile-detail">
                            <div class="profile-detail-small">Fecha de ingreso</div>
                            <div class="profile-detail-large">{{ checkout ? checkout.checkin.fecha_ingreso : 'Cargando' }}</div>
                        </div>
                        <div class="profile-detail">
                            <div class="profile-detail-small">Hora de Checkout</div>
                            <div class="profile-detail-large">
                                {{ checkout ? checkout.updated_at : 'Cargando' }}</div>
                        </div>
                        <div class="profile-detail">
                            <div class="profile-detail-small">Recepcionista de atención</div>
                            <div class="profile-detail-large">{{
                                checkout ? checkout.checkin.recepcionista.nombres : 'Cargando' }}</div>
                        </div>

                        <div class="profile-detail">
                            <div class="profile-detail-small">Empresa</div>
                            <div class="profile-detail-large">{{ checkout ? checkout.checkin.reserva.empresa.razon_social : 'Cargando' }}</div>
                        </div>

                
                        <div class="profile-detail">
                            <div class="profile-detail-small">Fecha Check Out</div>
                            <div class="profile-detail-large">{{ checkout ? checkout.checkin.reserva.datosReserva.fecha_checkout : 'Cargando' }}</div>
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        getCheckout() {
            axios.get("api/checkout/" + this.id, 
            
            ).then((value) => {
                console.log(value);
                this.checkout = value.data.data;
            })
        }
    }
}
